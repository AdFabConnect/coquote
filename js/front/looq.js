...
var hl = {
    ...
    bindMouseUp: function() {
        ...
        document.querySelector('body').addEventListener('mouseup', function(e) {
            ...
            if(e.target.classList.contains("bubble")) {
                hl.selectText(currentSelection);
                if(bubble != null) {
                    bubble.parentNode.removeChild(bubble);
                }
            }else {
                if(e.target.nodeName !== "INPUT" && e.target.nodeName !== "TEXTAREA") {
                    currentSelection = hl.up();
                   
                    chrome.extension.sendRequest({
                        msg: "copy",
                        text: currentSelection.copyString
                    });

                    if(currentSelection.plain !== "") {
                        if(typeof bubble !== "undefined" && bubble !== null) {
                            bubble.parentNode.removeChild(bubble);
                        }
                        hl.appendHTML(document.body, '<span class="bubble"><span class="btn">Send</span></span>');
    ...
    up:function()
    {
        ...
        // copyString = plain.replace(/\n/g, "<br />");
        // copyString = plain.replace(/\n/g, "<br />").replace(/ /g, "&nbsp;");
        copyString = plain.replace(/ /g, "&#160;").replace(/\n/g, "<br />");
        // debugger
        return {
